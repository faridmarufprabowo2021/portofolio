...
print("=== Refining Living Room: Detailed TV, PS5 Console, Soundbar & Front-Facing Sofa Angle ===")
...
m_tv_game_screen = create_pbr("Mat_TV_GameUI_Screen", (0.05, 0.45, 0.85, 1.0), roughness=0.08, emission=(0.05, 0.45, 0.85, 1.0), emission_str=2.5)
...
# -------------------------------------------------------------
# Sofa Base & Brass Legs (Sofa placed at y = -0.7 facing y = +3.0 TV)
add_beveled_box("Sofa_BaseFrame", (0, -0.7, 0.18), (2.4, 0.95, 0.14), radius=0.02, mat=m_sofa)

leg_positions = [(-1.1, -1.1, 0.06), (1.1, -1.1, 0.06), (-1.1, -0.3, 0.06), (1.1, -0.3, 0.06)]
for i, pos in enumerate(leg_positions):
    leg = add_cyl(f"Sofa_Leg_{i+1}", pos, r=0.03, d=0.12, mat=m_brass, col=col_furn)

# Backrest & Armrests
add_beveled_box("Sofa_Backrest", (0, -1.08, 0.58), (2.4, 0.22, 0.65), radius=0.06, mat=m_sofa)
add_beveled_box("Sofa_Armrest_L", (-1.12, -0.7, 0.45), (0.24, 0.95, 0.42), radius=0.05, mat=m_sofa)
add_beveled_box("Sofa_Armrest_R", (1.12, -0.7, 0.45), (0.24, 0.95, 0.42), radius=0.05, mat=m_sofa)

# 3 Plush Seat Cushions (Facing TV)
cushion_w = 0.64
for i, x_pos in enumerate([-0.66, 0, 0.66]):
    add_beveled_box(f"Seat_Cushion_{i+1}", (x_pos, -0.65, 0.30), (cushion_w, 0.78, 0.18), radius=0.05, mat=m_sofa)

# 3 Ergonomic Backrest Cushions with Button Tufting
for i, x_pos in enumerate([-0.66, 0, 0.66]):
    back_c = add_beveled_box(f"Back_Cushion_{i+1}", (x_pos, -0.96, 0.56), (cushion_w, 0.18, 0.48), radius=0.05, mat=m_sofa)
    back_c.rotation_euler = (math.radians(6), 0, 0)
    
    for b_x in [-0.15, 0.15]:
        bpy.ops.mesh.primitive_uv_sphere_add(radius=0.015, location=(x_pos + b_x, -0.91, 0.56))
...
p1 = add_beveled_box("Pillow_Mustard_L", (-0.9, -0.65, 0.48), (0.38, 0.14, 0.38), radius=0.06, mat=m_mustard)
p1.rotation_euler = (math.radians(15), math.radians(8), math.radians(15))

p2 = add_beveled_box("Pillow_Cream_L", (-0.75, -0.55, 0.46), (0.35, 0.12, 0.35), radius=0.05, mat=m_cream)
p2.rotation_euler = (math.radians(10), math.radians(-5), math.radians(-10))

p3 = add_beveled_box("Pillow_Teal_R", (0.85, -0.60, 0.48), (0.40, 0.14, 0.40), radius=0.06, mat=m_teal)
p3.rotation_euler = (math.radians(12), math.radians(-10), math.radians(-18))

# Knit Throw Blanket
blanket_top = add_beveled_box("Throw_Blanket_Drape", (1.08, -0.65, 0.47), (0.28, 0.65, 0.03), radius=0.015, mat=m_blanket)
blanket_top.rotation_euler = (math.radians(-5), 0, math.radians(-4))
add_beveled_box("Throw_Blanket_Fold", (1.23, -0.65, 0.30), (0.03, 0.62, 0.32), radius=0.015, mat=m_blanket)

# Area Rug
add_box("Area_Rug", (0, 0.2, 0.01), (2.8, 2.4, 0.015), m_rug, col_decor)

# Coffee Table & Decor
add_box("CoffeeTable_Top", (0, 0.45, 0.38), (1.1, 0.6, 0.04), m_wood, col_furn)
add_box("Table_Leg1", (-0.45, 0.20, 0.18), (0.05, 0.05, 0.36), m_brass, col_furn)
add_box("Table_Leg2", (0.45, 0.20, 0.18), (0.05, 0.05, 0.36), m_brass, col_furn)
add_box("Table_Leg3", (-0.45, 0.70, 0.18), (0.05, 0.05, 0.36), m_brass, col_furn)
add_box("Table_Leg4", (0.45, 0.70, 0.18), (0.05, 0.05, 0.36), m_brass, col_furn)

add_box("Decor_Book1", (-0.2, 0.45, 0.42), (0.22, 0.28, 0.03), m_mustard, col_decor)
add_box("Decor_Book2", (-0.2, 0.45, 0.45), (0.20, 0.25, 0.03), m_wall, col_decor)
add_cyl("Decor_Mug", (0.25, 0.50, 0.44), r=0.04, d=0.09, mat=m_pot, col=col_decor)
...
# Ultra-thin Metallic TV Bezel & Game Screen
...
# -------------------------------------------------------------
# PS5 Stand & Core Unit (Right side of TV cabinet)
...
# 3/4 Architectural Camera Shot (Revealing Front of Sofa, Table, TV & PS5)
cam_data = bpy.data.cameras.new("Archviz_LivingRoom_Cam")
cam_data.lens = 32
cam_obj = bpy.data.objects.new("Archviz_LivingRoom_Cam", cam_data)
cam_obj.location = (-2.3, -2.6, 1.40)
cam_obj.rotation_euler = (math.radians(78), 0, math.radians(-42))
...
out_blend = r"C:\Users\PERSONAL\Downloads\Denia\Cozy_Modern_LivingRoom_DetailedPS5.blend"
bpy.ops.wm.save_as_mainfile(filepath=out_blend)

print(f"✅ HIGH-DETAIL REFINED LIVING ROOM SAVED TO: {out_blend}")
