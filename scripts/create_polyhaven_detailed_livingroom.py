...
print("=== Constructing Living Room with Polyhaven Open-Source 3D Plant, Detailed Speaker & Cabinet Door Handles ===")
...
m_soil = create_pbr("Mat_DarkOrganicSoil", (0.12, 0.08, 0.05, 1.0), 0.95)
m_leaf = create_pbr("Mat_MonsteraLeaf", (0.06, 0.38, 0.09, 1.0), 0.35)
...
# DETAILED TV GAME HUD SCREEN & SOUNDBAR MATERIALS
...
# DETAILED SPEAKER / SOUNDBAR MATERIALS
...
# CABINET PROPS & HANDLES MATERIALS
...
# -------------------------------------------------------------
# 3. HIGH-DETAIL SMART TV, DETAILED SPEAKER & CABINET WITH DOOR HANDLES
# -------------------------------------------------------------
# Japandi Media Cabinet
add_box("TV_Cabinet_Frame", (0, 2.75, 0.25), (2.4, 0.45, 0.45), m_wood, col_furn)

# Slatted Wooden Sliding Doors (Left & Right)
...
# ELEGANT BRASS DOOR HANDLES / PULL KNOBS FOR OPENING & CLOSING DOORS
# Left Door Handle (Mounted vertically on the right edge of left door)
...
# Right Door Handle (Mounted vertically on the left edge of right door)
...
# Shelf Props: Audio Receiver & Terracotta Vases (Bottom Shelf)
...
# Ultra-thin Metallic TV Bezel & Game UI HUD Screen
...
# HIGH-DETAIL TV SOUNDBAR / SPEAKER
# Main Body & Perforated Front Grille
...
# Brass Trim Borders (Top & Bottom Edges)
...
# Status LED & Micro Buttons (Right side of Soundbar)
...
# -------------------------------------------------------------
# 5. IMPORT POLYHAVEN OPEN-SOURCE 3D PLANT MODEL
# -------------------------------------------------------------
poly_plant_path = r"C:\Users\PERSONAL\Downloads\Denia\polyhaven_plant\potted_plant_01.gltf"

if os.path.exists(poly_plant_path):
    print(f"🌿 Importing Polyhaven Open Source 3D Plant Model from {poly_plant_path}...")
    bpy.ops.import_scene.gltf(filepath=poly_plant_path)
    
    # Scale and position imported Polyhaven plant at left corner (x = -1.8, y = 2.3)
    plant_objects = [o for o in bpy.context.selected_objects if o.type == 'MESH']
    for p_obj in plant_objects:
        p_obj.location = (-1.8, 2.2, 0.0)
        p_obj.scale = (0.75, 0.75, 0.75) # Scale to fit room
        col_decor.objects.link(p_obj)
        if p_obj.name in scene.collection.objects:
            scene.collection.objects.unlink(p_obj)
else:
    print("⚠️ Polyhaven plant file not found, creating high-detail fallback plant.")
    add_cyl("Monstera_Pot", (-1.9, 2.3, 0.25), r=0.22, d=0.48, mat=m_pot, col=col_decor)

# Abstract Modern Geometric Art with Glass Frame
...
# -------------------------------------------------------------
# 6. LIGHTING & PERFECT FRONT 3/4 ARCHITECTURAL CAMERA
# -------------------------------------------------------------
...
# Wide 24mm Architectural Front-Side Perspective Camera
...
out_blend = r"C:\Users\PERSONAL\Downloads\Denia\Cozy_Modern_LivingRoom_PolyhavenPlant.blend"
bpy.ops.wm.save_as_mainfile(filepath=out_blend)

print(f"✅ POLYHAVEN PLANT & DETAILED SOUNDBAR/HANDLES LIVING ROOM SAVED TO: {out_blend}")
