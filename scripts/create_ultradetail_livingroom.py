...
print("=== Constructing Ultra-Detailed Living Room: Fenestrated Monstera, Game HUD TV Screen, Slatted Cabinet with Game Cases & Abstract Art ===")
...
# -------------------------------------------------------------
# 3. HIGH-DETAIL SMART TV (GAME HUD UI) & JAPANDI MEDIA CABINET
# -------------------------------------------------------------
# Japandi Media Cabinet with Open Center Shelves & Slatted Side Doors
...
# Game HUD UI Elements (HP Bar bottom-left, MiniMap top-right)
...
# -------------------------------------------------------------
# 5. REALISTIC MONSTERA DELICIOSA PLANT & ABSTRACT ART FRAME
# -------------------------------------------------------------
# Ribbed Ceramic Pot & Dark Soil
add_cyl("Monstera_Pot", (-1.9, 2.3, 0.25), r=0.22, d=0.48, mat=m_pot, col=col_decor)
add_cyl("Monstera_Soil", (-1.9, 2.3, 0.47), r=0.21, d=0.03, mat=m_soil, col=col_decor)

# Organic Fenestrated Monstera Leaves & Curved Stems
stem_positions = [
    (-1.9, 2.3, 0.70, 25, -15, 0.32, 0.40),
    (-1.9, 2.3, 0.85, 35, 20, 0.36, 0.44),
    (-1.9, 2.3, 0.95, 15, 45, 0.42, 0.48),
    (-1.9, 2.3, 1.10, -20, -30, 0.38, 0.42),
    (-1.9, 2.3, 1.20, 40, -45, 0.44, 0.50),
]

for i, (sx, sy, sz, rx, ry, lw, lh) in enumerate(stem_positions):
    # Stem
    add_cyl(f"Monstera_Stem_{i+1}", (sx, sy, sz - 0.2), r=0.015, d=0.4, mat=m_leaf, col=col_decor)
    
    # Fenestrated Leaf Body
    leaf = add_box(f"Monstera_Leaf_{i+1}", (sx + math.sin(i)*0.15, sy + math.cos(i)*0.15, sz), (lw, lh, 0.008), m_leaf, col_decor)
    leaf.rotation_euler = (math.radians(rx), math.radians(ry), math.radians(i*72))
    
    # Cutouts / Fenestrations (Simulated leaf splits)
    add_box(f"Monstera_Split_{i+1}", (sx + math.sin(i)*0.18, sy + math.cos(i)*0.18, sz), (lw*0.4, lh*0.2, 0.01), m_wall, col_decor)
...
# Camera: Wide 24mm Front 3/4 Perspective Angle (Capturing Plant, TV, Cabinet, PS5 & Sofa)
cam_data = bpy.data.cameras.new("Archviz_LivingRoom_Cam")
cam_data.lens = 24 # 24mm Wide Archviz Lens
...
out_blend = r"C:\Users\PERSONAL\Downloads\Denia\Cozy_Modern_LivingRoom_UltraDetail.blend"
bpy.ops.wm.save_as_mainfile(filepath=out_blend)

print(f"✅ ULTRA-DETAILED LIVING ROOM SCENE SAVED TO: {out_blend}")
